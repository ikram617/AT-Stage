...
class ATIDGenerator:
    ...
    @staticmethod
    def get_wilaya_code(name: str) -> str:
        """
        Retourne le code wilaya formatté {code}0.
        Ex: 'Oran' -> '310'
        """
        if not name: return "310"
        n = name.lower().strip()
        # Fallback si on a "31 - Oran"
        if " - " in name:
            try:
        ...
        for w in wilayas_algerie:
            if w["nom"].lower() == n:
                return f"{w['code']:02d}0"
        return "310"
    ...
    def _format_id(
        ...
        # Règle métier : COM(...) pour les commerces, Porte(...) pour les logements
        prefix = "COM" if usage == "commerces" else "Porte"
